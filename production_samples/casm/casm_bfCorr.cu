...
__global__ void sum_ib(half * dra, half * dia, half * drb, half * dib, half * dout, int * flagants) {
  ...
  int ELEMS = 16;  
  if(idx < ELEMS && false) {
    printf("(casm) SIC thread %d Input A_r = %f, Input A_i = %f, Input B_r = %f, Input B_i = %f\n", idx,
...
int main (int argc, char *argv[]) {  
  ...
  while ((arg=getopt(argc,argv,"c:i:o:t:f:a:s:g:p:kbdh")) != -1)
	...
	case 't':
	  if (optarg)
	      ...
	      if(DEBUG) printf("reading test file name from %s\n", optarg);
	      if (sscanf (optarg, "%s", &ftest) != 1) {
		syslog(LOG_ERR, "could not read test file name from %s\n", optarg);
		if(DEBUG) printf("could not read test file name from %s\n", optarg);
		return EXIT_FAILURE;
	      }
	      break;
	    }
	...
	case 'a':
	  if (optarg)
	      ...
	      if(DEBUG) printf("reading calib file from %s\n", optarg);
	...
	case 'f':
	  if (optarg)
	      ...
	      if(DEBUG) printf("reading flag ants file from %s\n", optarg);
  ...
  if(DEBUG) printf("Input parsing complete\n");
  ...
  if (bf) {
    ...
    if(DEBUG) printf("Setting up beamformer\n");
    ...
    fclose(ff);
    if(DEBUG) printf("Flag ants read\n");
  ...
  if(DEBUG) printf("Weight parsing complete\n");
  ...
  if (test) {
	...
	for(int i=0; i<n_test_rep; i++) dcorrelator(&d, nbase_val);
	...
	// Warm up
	dbeamformer(&d);
	for(int i=0; i<n_test_rep; i++) dbeamformer(&d);
	printf("beamformer complete\n");
	...
	for(int i=0*output_size/2 - 0; i<0*output_size/2 + 32; i++) {
	  printf("Beamformer out (casm) %d: %u\n", i, (unsigned char)output_data[i]);
	}
	...
	for(int i=output_size/2 - 8; i<output_size/2 + 8; i++) {
	  printf("Beamformer out (casm) %d: %u\n", i, (unsigned char)output_data[i]);
	}
  ...
  // finish up
  deallocate(&d,bf);  
  if(!test) casm_dbgpu_cleanup (hdu_in, hdu_out);
#ifdef USE_GGP
  endQuda();
#endif
}
