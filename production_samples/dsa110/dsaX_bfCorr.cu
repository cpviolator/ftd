...
void initialize(dmem * d, int bf, int subtract_ib) {
  ...
  if (bf==1) {
    ...
    // The 2 in the weights is for each arm
...
__global__ void transpose_fluff_bf(unsigned short * idata, half * dra, half * dia, half * drb, half * dib) {
  ...
  // DEBUG---------------
  int ELEMS = 32;
  int linear_idx_x = blockIdx.x * blockDim.x + threadIdx.x;
  int linear_idx_y = blockIdx.y * blockDim.y + threadIdx.y;
  int linear_idx   = gridDim.x * blockDim.x * linear_idx_y + linear_idx_x;

  // Ensure input fidelity
  if(linear_idx < ELEMS && false) {
    unsigned short val = idata[linear_idx];
    unsigned char tmp1 = val & 0xFF;
    unsigned char tmp2 = (val >> 8) & 0xFF;
    printf("(dsa) TF INPUT: (%d,%d) = %d dra %e, dia %e, drb %e, dib %e\n", linear_idx_x, linear_idx_y, linear_idx,
	   ((float)((char)((tmp1 & (unsigned char)(15)) << 4) >> 4)),
	   ((float)((char)((tmp1 & (unsigned char)(240))) >> 4)),
	   ((float)((char)((tmp2 & (unsigned char)(15)) << 4) >> 4)),
	   ((float)((char)((tmp2 & (unsigned char)(240))) >> 4)));
  }
  ...
  for (int j = 0; j < 32; j += 8) {
    tile[threadIdx.y+j][threadIdx.x] = idata[(y+j)*width + x];
    if(linear_idx < ELEMS && false) {
      printf("(dsa) Grabbing: thread %d from input %d tile(%d,%d)\n", linear_idx, (y+j)*width + x, threadIdx.y+j, threadIdx.x);
    }
  }
  ...
  for (int j = 0; j < 32; j += 8) {
    ...
    // Ensure output fidelity
    if(oidx < ELEMS && false) {
      printf("(dsa) TF OUTPUT: lin %d out %d dra %e, dia %e, drb %e, dib %e\n", linear_idx, oidx,
	     __half2float(dra[oidx]), __half2float(dia[oidx]), __half2float(drb[oidx]), __half2float(dib[oidx]));
    }     
  }
}
...
__global__ void power_sum_and_transpose_output(half * dra, half * drb, half * dia, half * dib, half * ibsum, int subtract_ib, half * outp) {
  ...
  int linear_idx_x = blockIdx.x * blockDim.x + threadIdx.x;
  int linear_idx_y = blockIdx.y * blockDim.y + threadIdx.y;
  int lin_idx = gridDim.x * blockDim.x * linear_idx_y + linear_idx_x;

  int low = 8192*100;
  int high = 8192*100 + 64;
  if(low <= lin_idx && lin_idx < high && false) {
    printf("(dsa) PST thread linear %d Input A_r = %f, Input A_i = %f, Input B_r = %f, Input B_i = %f, ib = %f\n", lin_idx,
	   __half2float(dra[lin_idx]), __half2float(dia[lin_idx]),
	   __half2float(drb[lin_idx]), __half2float(dib[lin_idx]),
	   __half2float(ibsum[lin_idx]));
  }    
  */  
  ...
  for (int j = 0; j < 32; j += 8) {
    ...
    /*
    if(low <= iidx && iidx < high && false) {
      printf("(dsa) PST thread tile %d Input A_r = %f, Input A_i = %f, Input B_r = %f, Input B_i = %f, ib = %f\n", iidx,
	     __half2float(dra[iidx]), __half2float(dia[iidx]),
	     __half2float(drb[iidx]), __half2float(dib[iidx]),
	     __half2float(ibsum[idx]));
    }    
    */
    ...
    /*
    if(low <= iidx && iidx < high && j == 0 && false) {
      printf("(dsa) PST thread %d iidx %d chan %d beam_outer %d beam_inner %d sum_ib_idx %d sum_ib %f %f %f %f sum_val %f\n",
	     lin_idx, iidx, iChan, iBeamSlow, iFast, idx,
	     __half2float(ibsum[4*idx]),
	     __half2float(ibsum[4*idx+1]),
	     __half2float(ibsum[4*idx+2]),
	     __half2float(ibsum[4*idx+3]),
	     __half2float(tile[threadIdx.y+j][threadIdx.x]));
    }      
    */
  }
  ...
  for (int j = 0; j < 32; j += 8) {
    ...
    /*
    if(low <= (y+j)*width + x && (y+j)*width + x < high && false) {
      printf("(dsa) PST thread %d out %d sum_val %f\n",
	     lin_idx, (y+j)*width + x, outp[(y+j)*width + x]);
    }        
    */
  }
}
...
__global__ void sum_transpose_and_scale_output(half * outp, unsigned char * odata, int subtract_ib) {
  ...
  int high = 0 + 1024;
  if(low <= lin_idx && lin_idx < high && false) {
    printf("(dsa) STS thread %d Input %f\n", lin_idx, __half2float(outp[lin_idx]));
  }    
  */
...
__global__ void sum_ib(half * dra, half * dia, half * drb, half * dib, half * dout, int * flagants) {
  ...
  if(idx < ELEMS && false) {
    printf("(dsa) SIC thread %d Input A_r = %f, Input A_i = %f, Input B_r = %f, Input B_i = %f\n", idx,
	   __half2float(dra[idx]), __half2float(dia[idx]),
	   __half2float(drb[idx]), __half2float(dib[idx]));
  }  
  */
  ...
  if (flagants[tid]==0) {
    ...
    /*
    printf("(dsa) SIC FLAGGED %d thread %d Input A_r = %f, Input A_i = %f, Input B_r = %f, Input B_i = %f\n", tid, idx,
	   __half2float(dra[idx]), __half2float(dia[idx]),
	   __half2float(drb[idx]), __half2float(dib[idx]));
    */
  }
  ...
  if (tid==0) {
    dout[bid] = ppsum[0];
    //if(bid < ELEMS && true) printf("(dsa) ib sum bidx %d: %f\n", bid, __half2float(dout[bid]));
  }
}
...
void dbeamformer(dmem * d) {
  ...
  if(false) {
    printf("BF: M = (NCHAN_SUM*2*NPACKETS_PER_BLOCK) = (%d * %d * %d) = %d\n", NCHAN_SUM, 2, NPACKETS_PER_BLOCK,m);
    printf("BF: N = (NBEAMS/2) = (%d/2) = %d\n", NBEAMS, n);
    printf("BF: K = (NANTS/2) = (%d/2) = %d\n", NANTS, k);
    
    printf("BF: LDA = k = %d\n", lda);
    printf("BF: LDB = k = %d\n", ldb);
    printf("BF: LDC = m = %d\n", ldc);
    
    printf("BF: STRIDE A = NCHAN_SUM*2*NPACKETS_PER_BLOCK*(NANTS/2) = (%d * 2 * %d * %d/2) = %d\n", NCHAN_SUM, NPACKETS_PER_BLOCK, NANTS, strideA);
    printf("BF: STRIDE B = (NBEAMS/2)*(NANTS/2) = (%d/2) * (%d/2) = %d\n", NBEAMS, NANTS, strideB);
    printf("BF: STRIDE C = (NBEAMS/2)*NCHAN_SUM*2*NPACKETS_PER_BLOCK = (%d/2) * %d * 2 * %d = %d\n", NBEAMS, NCHAN_SUM, NPACKETS_PER_BLOCK, strideC);
    
    printf("BF: BC =  NCHAN_PER_PACKET/NCHAN_SUM = (%d/%d) = %d\n", NCHAN_PER_PACKET, NCHAN_SUM, batchCount);
  }
  ...
  for (int iArm=0;iArm<2;iArm++) {
    ...
    for (i1=0;i1<NPACKETS_PER_BLOCK;i1++) 
      cudaMemcpyAsync(d->d_input+i1*(NANTS/2)*NCHAN_PER_PACKET*4,
		      d->h_input+i1*(NANTS)*NCHAN_PER_PACKET*4 + iArm*(NANTS/2)*NCHAN_PER_PACKET*4,
		      (NANTS/2)*NCHAN_PER_PACKET*4,cudaMemcpyHostToDevice,streams[iArm]);
    end = clock();
    d->cp += (float)(end - begin) / CLOCKS_PER_SEC;
    ...
    //printf("(dsa) Launching TF with (%d, %d) (%d, %d) = %llu threads \n", 32, 8, NCHAN_PER_PACKET*2/32, (NPACKETS_PER_BLOCK)*(NANTS/2)/32, (NCHAN_PER_PACKET*2) * (NPACKETS_PER_BLOCK)*(NANTS/8));    
    ...
    //printf("(dsa) Launching SIC with threads %d items %d\n", NCHAN_PER_PACKET*2*NPACKETS_PER_BLOCK * 32, NCHAN_PER_PACKET*2*NPACKETS_PER_BLOCK * 48);
    ...
    //printf("(dsa) Launching PST with threads %d\n", ((NPACKETS_PER_BLOCK/4) * NCHAN_PER_PACKET * NBEAMS * 8) / 32);
    ...
    //printf("(dsa) Launching STS with threads %d\n", (NBEAMS/2) * (NCHAN_PER_PACKET * (NPACKETS_PER_BLOCK/4)) / 32);
...
__global__ void populate_weights_matrix(float * antpos_e, float * antpos_n, float * calibs, half * war, half * wai, half * wbr, half * wbi, float * fqs, float dec) {
  ...
  int low = 2*((NCHAN_PER_PACKET/8)*(NBEAMS/2)*(NANTS/2)) - 1024;
  int high= 2*((NCHAN_PER_PACKET/8)*(NBEAMS/2)*(NANTS/2)) + 0;
  
  if(low <= inidx && inidx < high && false) {
    printf("%d of %d (dsa): weights A = (%f,%f) weights B = (%f,%f)\n", inidx, gridDim.x * blockDim.x, __half2float(war[inidx]), __half2float(wai[inidx]), __half2float(wbr[inidx]), __half2float(wbi[inidx])) ;
  }
}
...
void calc_weights(dmem * d) {
  ...
  //for(int i=0; i<NANTS; i++) printf("FLAGVAL(%d) = %d\n", i, flagas[i]);
  //for(int i=0; i<NANTS*(NCHAN_PER_PACKET/8)*2; i++) printf("CALIB(%d) = (%f,%f)\n", i, calibrations[2*i], calibrations[2*i]);  
  //for (int i=0;i<NANTS*(NCHAN_PER_PACKET/8)*2; i++) printf("CALIBS(%d) = (%f,%f)\n", i, calibs[2*i], calibs[2*i+1]);
...
int main (int argc, char *argv[]) {  
  ...
  if (bf) {
    ...
    if (!(ff=fopen(fcalib,"rb")) && !test) {
      syslog(LOG_ERR,"could not open calibss file\n");
      printf("could not open calibss file\n");
      exit(1);
    } else if (test) {
      printf("constructing dummy calibss file\n");
      for(int i=0; i<NANTS*2+NANTS*(NCHAN_PER_PACKET/8)*2*2; i++) {
	d.h_winp[i] = 1.0/(i+1);
      }
      printf("constructed dummy calibss file\n");
    } else {
      printf("reading calibss file\n");
  ...
  char * output_data, * o1;
  char * output_data_ggp;

  int nreps = 1, nchunks = 1;
  if (test) {

#ifdef USE_GGP
    // initialize MPI, QUDA comms grid and RNG (host_utils.cpp)
    //initComms(argc, argv, gridsize_from_cmdline);
    //int const commDims[4] = {0,0,0,0};
    //QudaCommsMap func = NULL;
    ...
    bf_param.packet_format = QUDA_PACKET_FORMAT_DSA110;
    bf_param.n_payload = 1;
    bf_param.n_time_per_payload = NTIME_INNER*NPACKETS_PER_BLOCK;
    bf_param.n_antennae_per_payload = NANTS;
    bf_param.n_channels_per_payload = NCHAN_PER_PACKET;
    bf_param.n_channels_inner = NCHAN_SUM;
    bf_param.n_polarizations = NPOL;
    bf_param.n_time_inner = NTIME_INNER;
    bf_param.n_beam = NBEAMS;
    bf_param.n_time_power_sum = 4;
    bf_param.n_flags = d.nflags;
    bf_param.sfreq = sfreq;
    bf_param.wfreq = 250.0/1024.0;
    bf_param.declination = mydec;
    bf_param.beam_separation = sep;
    bf_param.beam_separation_ns = sep_ns;
    bf_param.compute_prec = QUDA_SINGLE_PRECISION;
    bf_param.output_prec = QUDA_SINGLE_PRECISION;
    bf_param.data_type = QUDA_BLAS_DATATYPE_QC;
    bf_param.data_order = QUDA_BLAS_DATAORDER_ROW;
    bf_param.verbosity = QUDA_VERBOSE;
    bf_param.struct_size = sizeof(bf_param);
    
    printBeamformerParam(&bf_param);
    
    ggp::VoltageBeamformer110 ggp_bf(bf_param);
    
    // This will also compute frequencies and load flagant data
    ggp_bf.compute_weights_by_arm(d.h_winp, d.flagants, d.nflags, d.obsdec, sfreq);
    ...
    printf("in_block_size = %lu = NPACKETS_PER_BLOCK * NANTS * NCHAN_PER_PACKET * NPOL * NTIME_INNER = %d * %d * %d * %d * %d\n", in_block_size, NPACKETS_PER_BLOCK, NANTS, NCHAN_PER_PACKET, NPOL, NTIME_INNER);
	...
	fread(d.h_input+chunks*rd_size,rd_size,1,fin);
      ...
      //printf("Raw Packet Voltage (dsaX) %d: (%f,%f)\n", pidx,
      //(float)((char)(((unsigned char)(d.h_input[pidx]) & (unsigned char)(15)) << 4) >> 4),
      //(float)((char)(((unsigned char)(d.h_input<[pidx]) & (unsigned char)(240))) >> 4)); 
      //}
#endif
...
#ifdef USE_GGP
	output_data_ggp = (char *)malloc(output_size);
	// warm up
	ggp_bf.compute_by_arm((void*)output_data_ggp, (void*)d.h_input);
	
	// bench
	begin = clock();
	for(int rep=0; rep<n_test_rep; rep++) ggp_bf.compute_by_arm((void*)output_data_ggp, (void*)d.h_input);
	end = clock();
	float bftime = (float)(end - begin) / CLOCKS_PER_SEC;
	
	fout = fopen("output_ggp.dat","ab");
	fwrite((unsigned char *)output_data_ggp,sizeof(unsigned char),output_size,fout);
	fclose(fout);

	for(int i=output_size/2 - 32; i<output_size/2 + 32; i++) {
	  printf("Beamformer out (ggp) %d: %u\n", i, (unsigned char)output_data_ggp[i]);
	}
	printf("GGP beamformer time = %f (%f per call) secs\n", bftime, bftime/n_test_rep);
#else
	// warm up
	dbeamformer(&d);
	...
	// bench
	begin = clock();
	for(int rep=0; rep<n_test_rep; rep++) dbeamformer(&d);

	// Include data transfer to host, as GGP does that for the user.
  ...
  // finish up
  deallocate(&d,bf);
  if(!test) dsaX_dbgpu_cleanup (hdu_in, hdu_out);
#ifdef USE_GGP
  endQuda();
#endif
}
