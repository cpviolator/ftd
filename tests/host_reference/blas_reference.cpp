                           ...
                           uint64_t refB_size, uint64_t refC_size, QudaBLASParam *blas_param)
{
  ...
  //printfQuda("Eigen m, n, k = %d %d %d\n", m, n, k);
  //printfQuda("Eigen lda, ldb, ldc = %d %d %d\n", lda, ldb, ldc);
  ...
  for (int batch = 0; batch < batches; batch += max_stride) {
    ...
    printf("Eigen batch %d: (%f,%f) (%f,%f)\n", batch, A(0,0).real(), A(0,0).imag(), B(0,0).real(), B(0,0).imag());
