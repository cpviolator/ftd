...
namespace ggp {
  ...
  VoltageBeamformer110::VoltageBeamformer110(BeamformerParam &param) {
    ...
    n_weights_per_pol = n_freq * n_antennae_per_payload * n_beam/2;
    logQuda(QUDA_VERBOSE,"n_weights_per_pol = %lu\n", n_weights_per_pol); 
    ...
    // N complex elements in input block, also size in bytes.
    ...
    result_payload_size = 2 * compute_data_size * (n_channels_per_payload *
						   n_beam *
						   n_time_per_payload);
    ...
    // Power sum complex elemets
    ps_data_elems = (n_time_per_payload/n_time_power_sum) * n_channels_per_payload * n_beam/2;
  ...
  void VoltageBeamformer110::refresh_weights(void *weights_data_A_host, void *weights_data_B_host) {
    ...
    logQuda(QUDA_VERBOSE, "Refreshed weights.\n");
  }
  
  void VoltageBeamformer110::refresh_flagants(void *flagants_data_host) {
    qudaMemcpy(flagants_data, flagants_data_host, n_antennae_per_payload * sizeof(int), qudaMemcpyHostToDevice);
    logQuda(QUDA_VERBOSE, "Refreshed flagants.\n");
  }

  void VoltageBeamformer110::init_memory() {
    if(!mem_init) {
      logQuda(QUDA_VERBOSE, "Initialising memory in voltage_bf_dsa110\n");
      blas_param = newQudaBLASParam();

      // DSA110 specialities
      input_copy_110 = device_pinned_malloc(in_payload_size/2);
      ...
      // 2*in_payload_size for complex, /2 for split polarization
      ...
      mem_init = true;
      logQuda(QUDA_VERBOSE, "Memory init complete in voltage_bf_dsa110\n");
    }
  }
  
  void VoltageBeamformer110::destroy_memory() {
    if(mem_init) {
  ...
  void VoltageBeamformer110::populate_blas_param(BeamformerParam &param) {
    ...
    blas_param.m = param.n_beam;
    blas_param.n = param.n_channels_inner * param.n_time_per_payload;
    blas_param.k = param.n_antennae_per_payload;
  ...
  void VoltageBeamformer110::refresh_input_by_arm(void *input_data_host, uint64_t stream_idx) {

#if 0
    // Inspect incoming packet
    char *p = (char*)input_data_host;
    for(int pidx = 0; pidx<128; pidx++) {
      printf("Raw Packet Voltage (ggp) %d: (%f,%f)\n", pidx,
	     (float)((char)(((unsigned char)(p[pidx]) & (unsigned char)(15)) << 4) >> 4),
	     (float)((char)(((unsigned char)(p[pidx]) & (unsigned char)(240))) >> 4)); 
    }
    // Good
#endif
    ...
    qudaMemcpy(input_copy, input_data_host, in_payload_size, qudaMemcpyHostToDevice);

    logQuda(QUDA_VERBOSE, "Populating two (32b,32b) complex float arrays of length %lu bytes from (4b,4b) char array of length %lu bytes\n",
	    2*sizeof(float)*in_payload_size/n_pol, in_payload_size);
	...
	int inc_size = (n_antennae_per_payload/2)*n_channels_per_payload*n_time_inner * n_pol;
	int cur_inc = 0;
	
	for (uint64_t i=0; i<n_time_per_payload/n_time_inner; i++) {
	  qudaMemcpyAsync((char*)input_copy_110 + i * (n_antennae_per_payload/2) * n_channels_per_payload * n_time_inner * n_pol,
			  (char*)input_copy + (i * n_antennae_per_payload * n_channels_per_payload * n_time_inner * n_pol +
					       iArm * (n_antennae_per_payload/2) * n_channels_per_payload * n_time_inner * n_pol),
			  (n_antennae_per_payload/2) * n_channels_per_payload * n_time_inner * n_pol,
			  qudaMemcpyDeviceToDevice, get_stream(stream_idx));
	  //printf("current increment: %d %d %d\n", iArm, i, cur_inc);
	  cur_inc += 2*inc_size + iArm*inc_size;
	}
	reorderDataPolT((float*)input_data_full_ptr_A.data() + iArm*(in_payload_size/2),
			(float*)input_data_full_ptr_B.data() + iArm*(in_payload_size/2),
			(char*)input_copy_110, n_time_per_payload, n_time_inner, n_channels_per_payload, n_pol, n_antennae_per_payload/n_arm, compute_prec, stream_idx);	  
	
      }
      break;
    case QUDA_PACKET_FORMAT_DSA2K:
      errorQuda("Incorrect packet format %d\n", packet_format);
      break;
    default: errorQuda("Unknown packet format %d\n", packet_format);
    }
  }
  
  void VoltageBeamformer110::compute_by_arm(void *output_data_host, void *input_data_host) {

    blas_param.data_order = QUDA_BLAS_DATAORDER_ROW;
    blas_param.blas_type = QUDA_BLAS_GEMM;
    blas_param.trans_a = QUDA_BLAS_OP_N;
    blas_param.trans_b = QUDA_BLAS_OP_T;
    blas_param.m = n_beam/2;
    blas_param.n = n_channels_inner * n_time_per_payload;
    blas_param.k = n_antennae_per_payload/2;
    blas_param.alpha = 1.0;
    blas_param.beta = 0.0;
    blas_param.lda = blas_param.k;
    blas_param.ldb = blas_param.k;
    ...
    // Inner sum parameters
    n_thread_sict = (((n_time_per_payload/(n_time_inner*n_time_power_sum)) *
		      (n_channels_per_payload / n_channels_inner) * n_beam)/2);
    
    // Compute pointer offsets (in bytes) for second arm.
    uint64_t input_offset = (in_payload_size * compute_data_size)/2;
    uint64_t result_offset = result_payload_size/2;
    uint64_t output_offset = out_payload_size/2;
    
    uint64_t weights_offset = (2 * n_weights_per_pol * sizeof(float))/2;
    uint64_t flagants_offset = (n_antennae_per_payload * sizeof(int))/2;
    ...
    for (uint64_t stream_idx = 0; stream_idx<n_payload; stream_idx++) {
      ...
      // in the second half
      refresh_input_by_arm(input_data_host, stream_idx);
	...
	powerSum((char*)ps_data,
		 ...
		 (char*)result_data_B + result_offset*iArm,
		 (char*)ib_sum_data,
		 ps_data_elems, n_beam/2, n_time_power_sum, compute_prec, stream_idx,
	...
	sumInnerChanTimeT((char*)output_copy + output_offset*iArm,
			  (char*)ps_data,
			  n_thread_sict, (n_time_per_payload/n_time_inner)/4, n_beam/2, n_channels_per_payload, n_channels_inner, n_time_inner, compute_prec, stream_idx);    
	
	qudaMemcpyAsync((char*)output_data_host + output_offset*iArm,
			(char*)output_copy + output_offset*iArm, out_payload_size/2, qudaMemcpyDeviceToHost, get_stream(stream_idx));
      }
      //qudaDeviceSynchronize();
    }
  }

  void VoltageBeamformer110::compute_weights_by_arm(void *input_data_host, void *flagants_host, int n_flags, float dec, float sfreq) {
    ...
    int64_t n_threads = 2*(n_channels_per_payload/n_channels_inner)*(n_beam/2)*(n_antennae_per_payload/2);
    
    logQuda(QUDA_VERBOSE, "Launching computeWeights with %lu threads\n", n_threads); 
    computeWeights(weights_data_A, weights_data_B, dev_ant_pos_E, dev_ant_pos_N, dev_calibrations, flagants_data, dev_freq, 37.23 - dec, 2, n_threads, compute_prec);
  ...
  VoltageBeamformer110::~VoltageBeamformer110() {
    destroy_memory();
  }
}
