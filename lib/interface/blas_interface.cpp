...
void blasGEMMQuda(void *arrayA, void *arrayB, void *arrayC, QudaBoolean use_native, QudaBLASParam *blas_param)
{
  ...
  if (use_native == QUDA_BOOLEAN_FALSE) {
    ...
    case QUDA_BLAS_DATATYPE_C:
      ...
      break;
    case QUDA_BLAS_DATATYPE_H:
      data_size = sizeof(short);
      break;
    default:
      errorQuda("Unknown BLAS data type %d", blas_param->data_type);
    }
    ...
    case QUDA_BLAS_ENGINE_CUTLASS:
      stridedBatchGEMMCutlass(A_d, B_d, C_d, *blas_param, QUDA_CUDA_FIELD_LOCATION);
      break;
    case QUDA_BLAS_ENGINE_TCC:
      errorQuda("TCC engine does not support the generic BLAS GEMM interface. "
                "Use the XEngine path with engine = QUDA_BLAS_ENGINE_TCC instead.");
      break;
    default: errorQuda("Unknown BLAS engine %d", blas_param->engine);
    }
