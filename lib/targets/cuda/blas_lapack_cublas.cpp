...
namespace quda
{
  ...
  /*
  namespace ggp_cutlass
  {
    long long stridedBatchGEMMCutlass(void *A_data, void *B_data, void *C_data, QudaBLASParam blas_param,
				      QudaFieldLocation location)
    {
#ifdef CUTLASS_LIB      
      // Volta Tensor Core operations exposed with mma.sync are first available in CUDA 10.1.
      // CUTLASS must be compiled with CUDA 10.1 Toolkit to run these interfaces.
      if (!(__CUDACC_VER_MAJOR__ > 10 || (__CUDACC_VER_MAJOR__ == 10 && __CUDACC_VER_MINOR__ >= 1))) {
	errorQuda("Volta Tensor Core operations must be compiled with CUDA 10.1 Toolkit or later.");
      }
...
// Matrix A, Row Major for Matrix B and Row Major for Matrix C
      ...
      errorQuda("CUTLASS interface not implemented");
#else
  ...
  */
  ...
  namespace blas_lapack
  {
	...
	cublasSetStream(handle, quda::target::cuda::get_stream(quda::device::get_stream(blas_param.stream_idx))); 
	...
	if (getVerbosity() >= QUDA_DEBUG_VERBOSE) printfQuda("Deduced max_stride = %d\n", max_stride);
	...
	if (getVerbosity() >= QUDA_DEBUG_VERBOSE) printfQuda("Actual max_stride = %d\n", max_stride);
	
        // The number of GEMMs to compute
        const uint64_t batch = blas_param.batch_count / max_stride;
	if (getVerbosity() >= QUDA_DEBUG_VERBOSE) printfQuda("Actual GEMMS = %d / %d = %lu\n", blas_param.batch_count, max_stride, batch);
	...
	if (getVerbosity() >= QUDA_DEBUG_VERBOSE) printQudaBLASParam(&blas_param); 	  
	...
	//---------------
	  ...
	  flops += batch * 2 * blas_param.m * blas_param.n * blas_param.k;
	  ...
	  prec_char = 'S';
          ...
          flops += batch * FLOPS_SGEMM(blas_param.m, blas_param.n, blas_param.k);
        } else if (blas_param.data_type == QUDA_BLAS_DATATYPE_H) {
	  // REAL HALF
	  //----------
          typedef half H;
	  
          const H alpha = __double2half(static_cast<std::complex<double>>(blas_param.alpha).real());
          const H beta = __double2half(static_cast<std::complex<double>>(blas_param.beta).real());
	  
	  cublasStatus_t error;
	  
	  if (batch > 1) {
	    error = cublasHgemmStridedBatched(handle, trans_a, trans_b, blas_param.m, blas_param.n, blas_param.k,
                                              &alpha, (H *)A_d + blas_param.a_offset, blas_param.lda, a_stride,
                                              (H *)B_d + blas_param.b_offset, blas_param.ldb, b_stride, &beta,
                                              (H *)C_d + blas_param.c_offset, blas_param.ldc, c_stride, batch);

            if (error != CUBLAS_STATUS_SUCCESS)
              errorQuda("\nError in cuBLASHGEMMStridedBatched, error code = %d\n", error);
          } else {
            error = cublasHgemm(handle, trans_a, trans_b, blas_param.m, blas_param.n, blas_param.k, &alpha,
                                (H *)A_d + blas_param.a_offset, blas_param.lda, (H *)B_d + blas_param.b_offset,
                                blas_param.ldb, &beta, (H *)C_d + blas_param.c_offset, blas_param.ldc);

            if (error != CUBLAS_STATUS_SUCCESS) errorQuda("\nError in cuBLASHGEMMBatched, error code = %d\n", error);
          }
          flops += batch * FLOPS_SGEMM(blas_param.m, blas_param.n, blas_param.k);
        } else {
        ...
        if (getVerbosity() >= QUDA_DEBUG_VERBOSE)
          printfQuda("Batched matrix %cGEMM completed in %f seconds with GFLOPS = (%f/%f) = %f\n", prec_char, time, 1e-9 * flops, time, 1e-9 * flops / time);
        //-------------------------------------------------------------------------
