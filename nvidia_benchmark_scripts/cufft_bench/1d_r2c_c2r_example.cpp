...
int main(int argc, char *argv[]) {
    ...
    CUDA_RT_CALL(cudaMemcpy(d_input, input.data(), sizeof(input_type) * input.size(),
			    cudaMemcpyHostToDevice));

    // out-of-place Forward transform
    CUFFT_CALL(cufftExecR2C(planr2c, d_input, d_output));

    CUDA_RT_CALL(cudaMemcpy(output.data(), d_output, sizeof(output_type) * output.size(),
    ...
    // Normalize the data
    scaling_kernel<<<1, 128>>>(d_output, element_count, 1./fft_size);
    ...
    for(int i = 0; i < fft_size; i++) {
      error += pow(input[i] - input_orig[i],2);
      std::printf("error = %f\n", pow(input[i] - input_orig[i],2));
    }
